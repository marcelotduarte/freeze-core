...
class BuildBases(setuptools.command.build_ext.build_ext):
    ...
    def _regen_frozen(self) -> None:
        """Regenerate frozen modules to freeze_core wheel.
        ...
        if self.inplace:
            return  # For the editable wheel (develop mode), that's all.
        # Copy the autogenerated file to the wheel.
        freeze_core_dir = f"{self.build_lib}/freeze_core"
        target_path = f"{freeze_core_dir}/frozen"
        self.mkpath(target_path)
        self.copy_file(
            f"src/freeze_core/frozen/frozen-{SOABI}.json",
            f"{target_path}/frozen-{SOABI}.json",
        )
